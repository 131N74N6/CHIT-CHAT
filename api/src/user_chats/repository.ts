...
class UserChatRepository {
    ...
    async deleteAllMessagesPermanently(ids: ObjectId[]) {
        return await this.user_chats.deleteMany({ _id: { $in: ids } });
    }

    async deleteAllMessagesTemporary(ids: ObjectId[]) {
        return await this.user_chats.updateMany({ _id: { $in: ids }}, {
    ...
    async findChosenMessagesById(message_ids: string[]) {
    ...
    async showChosenMessageFiles(id: string) {
        return await this.user_chats.findOne({ _id: new ObjectId(id) }, { projection: { files: 1 }});
    }
}
